...
class DrSpaam(nn.Module):
    def __init__(
        self, dropout=0.5, num_pts=48, alpha=0.5, embedding_length=128, window_size=7
    ):
        ...
        # spatial attention
        self.memory_net = _SpatialAttentionMemory(
    ...
    def forward(self, x, inference=False):
        ...
        for i in range(n_scan):
            ...
            out, sim = self.memory_net(out)  # (B, CT, C, L)
...
class _SpatialAttentionMemory(nn.Module):
    def __init__(self, n_pts, n_channel, embedding_length, alpha, window_size):
